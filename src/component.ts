  ...
  renderTpl() {
    const src =
      this.answers.sourceType === "npm"
        ? path.join(this.installPath, "node_modules", this.templatePkg, "projectTemplate")
        : path.join(this.gitInstallPath, "packages", this.originTemplatePkg, "projectTemplate");
...
export default async (answers) => {
  ...
  new InitComponent({
    ...
    templatePkg: `${result.packageType}-component-template`,
